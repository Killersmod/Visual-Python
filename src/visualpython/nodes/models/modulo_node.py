"""
Modulo node model for modulo (remainder) operations in VisualPython.

This module defines the ModuloNode class, which performs modulo operations on two numeric values.
"""
...
class ModuloNode(BaseNode):
    """
    A node that computes the remainder of division (modulo operation).

    The ModuloNode provides a visual way to perform modulo operations without writing code.
    It accepts two numeric inputs and outputs the remainder when a is divided by b.
    ...
    """Unique identifier for modulo nodes."""
    ...
    def __init__(
        ...
        """
        Initialize a new ModuloNode instance.

        Args:
            node_id: Optional unique identifier. If not provided, a UUID will be generated.
            name: Optional display name. If not provided, defaults to 'Modulo'.
    ...
    def _setup_ports(self) -> None:
        """
        Set up the input and output ports for the modulo node.
    ...
    def validate(self) -> List[str]:
        ...
        # Warn about modulo by zero in defaults
        if self._default_b == 0:
            errors.append("Default divisor 'b' is zero, which will cause modulo by zero error")
    ...
    def execute(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with 'result' containing the remainder.
    ...
    def _get_serializable_properties(self) -> Dict[str, Any]:
        """
        Get modulo node specific properties for serialization.
    ...
    def _load_serializable_properties(self, properties: Dict[str, Any]) -> None:
        """
        Load modulo node specific properties from serialized data.
    ...
    def __repr__(self) -> str:
        """Get a detailed string representation of the modulo node."""
        return (
