"""
While Loop node visual widget with custom visual representation.

This module provides a specialized NodeWidget for WhileLoopNode that displays
the condition field and highlights the loop body output port to make the
condition-based iteration structure visible in the graph.
"""
...
class WhileLoopNodeWidget(NodeWidget):
    """
    Specialized visual representation of a WhileLoopNode.
    ...
    - Clear labeling of the loop body path with "LOOP BODY →" indicator

    The widget renders an additional configuration section showing the
    condition expression, making the while loop structure more visible.
    """
    ...
    def __init__(
        ...
        """
        Initialize a while loop node widget.

        Args:
            node: The WhileLoopNode model this widget represents.
            parent: Optional parent graphics item.
        """
        super().__init__(node, parent)

        # Store typed reference to the while loop node
        self._while_loop_node: WhileLoopNode = node
    ...
    def _calculate_size(self) -> None:
        ...
        # Add extra height for the condition section and loop body label section
        self._height += self.CONDITION_SECTION_HEIGHT + self.LOOP_BODY_SECTION_HEIGHT
    ...
    def _position_ports(self) -> None:
        ...
        # Position with offset for the loop body section label
    ...
    def paint(
        ...
        """Paint the while loop node widget with custom sections."""
        ...
        # Draw "Loop Body" section header with greenish background (like for loop)
        loop_body_rect = QRectF(
            0, condition_bottom_y, self._width, self.LOOP_BODY_SECTION_HEIGHT
        )
        ...
        loop_body_bottom_y = condition_bottom_y + self.LOOP_BODY_SECTION_HEIGHT
        painter.setPen(QPen(self._border_color, 1))
        painter.drawLine(
    ...
    def _get_condition_display_text(self) -> str:
        ...
        if self._while_loop_node.condition_code:
            # Truncate long condition code
            code = self._while_loop_node.condition_code
            if len(code) > 22:
                code = code[:19] + "..."
            return f"while {code}:"
        else:
